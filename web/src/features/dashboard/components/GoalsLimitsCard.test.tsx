...
describe("GoalsLimitsCard", () => {
  it("selects two most urgent limits and the nearest goal", () => {
    expect(
      selectDashboardProgress(goals, limits).map((item) => item.data.id),
    ).toEqual(["danger", "warning", "nearest"]);
  });

  it("fills all rows from the available type", () => {
    expect(
      selectDashboardProgress([], limits).map((item) => item.data.id),
    ).toEqual(["danger", "warning", "safe"]);
    expect(
      selectDashboardProgress(goals, []).map((item) => item.data.id),
  ...
  it("caps accessible progress and opens goals from a row", async () => {
    ...
    expect(
      screen.getByRole("progressbar", { name: "Subscriptions: 110%" }),
    ...
    expect(
      screen.getByRole("progressbar", { name: "Transport: 83%" }),
    ).toHaveAttribute("aria-valuenow", "83");
    expect(
      screen.getByRole("progressbar", { name: "Nearest goal: 100%" }),
    ).toHaveAttribute("aria-valuenow", "100");
